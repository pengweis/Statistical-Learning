...
## 5. TRAIN NN MODEL
set.seed(2)
NN = neuralnet(crim ~ zn + indus + chas + nox + rm + age + dis + rad + tax + ptratio + black + lstat + medv, train_scaled_NN, hidden = 5 , linear.output = TRUE, threshold=0.01 )
NN$result.matrix
plot(NN)
...
predict_test_scaled_NN <- data.frame(actual = test_scaled_NN$crim, prediction = predict_test_scaled_NN$net.result)
...
R_squared = 1 - sum((actual_test_NN-predict_test_NN)^2)/sum((actual_test_NN - mean(actual_test_NN))^2)
print('RMSE = ')  
RMSE
print('R squared = ')  
R_squared
